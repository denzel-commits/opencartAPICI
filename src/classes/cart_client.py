...
from src.classes.custom_assert.cart_assert_helper import CartAssertHelper
...
class CartClient:
    ...
    def remove_product_api(self, cart_id: int = None) -> CartAssertHelper:

        data = {"key": cart_id} if cart_id is not None else {}

        return CartAssertHelper(*self.api_client.post(Routes.CART + "/remove", data=data))

    def edit_product_api(self, product_data: dict) -> CartAssertHelper:
        return CartAssertHelper(*self.api_client.post(Routes.CART + "/edit", data=product_data))
